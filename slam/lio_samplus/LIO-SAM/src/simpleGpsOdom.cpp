...
#include "utility.h"
double yaw;
double x_1,y_1,dx,dy;
class GNSSOdom : public ParamServer {
public:
    GNSSOdom(ros::NodeHandle &_nh) {
        nh = _nh;
        gpsSub = nh.subscribe(navsatTopic, 1000, &GNSSOdom::GNSSCB, this,
...
private:
    void GNSSCB(const sensor_msgs::NavSatFixConstPtr &msg) {
        ...
        dx=-1.2;
        dy=0.0;
        x_1=enu(0) - dx*cos(yaw)+dy*sin(yaw);
        y_1=enu(1) - dx*sin(yaw)-dy*cos(yaw);
        ...
        odom_msg.pose.pose.position.x = x_1;//enu(0);
        odom_msg.pose.pose.position.y =y_1;//enu(1);
        ...
        pose.pose.position.x =x_1 ;//enu(0);
        pose.pose.position.y =y_1;// enu(1);
